...
def Hive():
    ...
    screen = pg.display.set_mode((880, 830))
    ...
    state = Game_State(initialize_grid(HEIGHT - 200, WIDTH, radius=18))
    ...
    while state.running:
        ...
        while state.main_loop:
            # print(state.moving_piece)
            # print(len(state.get_tiles_with_pieces(include_inventory=False)))
            pos = pg.mouse.get_pos()
            
            # comp random move
            if state.comp and state.turn % 2 == 0:
                random_move = ts.random_move(state)
                # step = random.choice(random_move)
                step = ts.nearest_to_queen_hex(random_move, state)
            ...
            # comp minimax move
            # if state.comp and state.turn % 2 == 0:
            #     step, _ = ts.minimax(state, 2, -math.inf, math.inf)
            #     step[0].move_piece(step[1])
            #     state.next_turn()
            #     # if player_has_no_moves(state) and state.turn > 7:
            #     #     state.open_popup()
            ...
            for event in pg.event.get():
                ...
                if event.type == pg.MOUSEBUTTONUP:
                    ...
                    if state.moving_piece and state.is_player_turn():
                        ...
                        print(state.turn, '---', old_tile.axial_coords, new_tile.axial_coords)
                        print(state.turn, '---', old_tile.coords, new_tile.coords)
